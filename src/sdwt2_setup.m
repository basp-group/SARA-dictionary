...
% ----------
% N : int[2]
%     Full image dimension. [1,2]
% I : int[:, 2]
%     Start index of each image tile (using 0-indexing) [Q,2]
% dims : int[:, 2]
%     Dimensions of the image tiles. [Q,2]
% J : int
%     Number of wavelet decomposition levels.
% wavelet : cell{:, 1} of string
%     Name of the wavelet dictionaries. {M,1}
% L : int[:]
%     Size of the wavelet filters. [M,1]
%
% Returns
% -------
% I_overlap_ref : array
%     start index of the facet within the full  image. [Q,2]
% dims_overlap_ref : array
%     Dimension of the facet (i.e., with  overlap) to be extracted from the
%     full image. [Q,2]
% I_overlap : array
%     Start index of the facet (for each  dictionary) within the full
%     image. {Q,1}[M,2]
% dims_overlap : cell{:, 1} of array
%     Dimension of the facet considered by each dictionary (including 0
%     coefficients from padding). {Q,1}[M,2]
% status : array
%     Status of the facet along each dimension (i.e., whether it is the
%     'first' facet, 'last', 'none' of the previous option or 'both'). The
%     associated status value are respectively -1, 1, 0 and NaN. [Q,2]
% offset : array
%     Offset to get the index of the first element (along each dimension)
%     to be considered in the facet for each wavelet dictionary.
% pre_offset : array
%     Number of coefficients to be cropped from the beginning of the global
%     facet [M,2]. (offset to get position of the first non-zero element in
%     the "global" facet)
% post_offset : array
%     Number of coefficients to be cropped from the end of the global facet
%    [M,2]. (offset to get position of the last non-zero element in the
%    "global" facet)
% Ncoefs : cell
%     Size of the details coefficients for each dictionary at each level of
%     the decomposition {Q}[(M-1)*(J+1)+1,2] if Dirac dict. is present.
% pre_offset_dict : array
%     For each dictionary, number of coefficients to be cropped from the
%     beginning of  the global facet [M,2].
% post_offset_dict : array
%     For each dictionary, number of coefficients to be cropped from the
%     end of the global facet [M,2].
%
% Note
% -----
% All indexes reported above (``I_*``) start from 0 (not from 1).
%
...
max_overlap = (2^J - 1) * (max(L(:)) - 1);
err_check = any(dims <= max_overlap, 1);
if any(err_check)
    disp(['sdwt2_setup: Overlap required by sdwt2 is: ', num2str(max_overlap), 'along each dimension.'])
    error('sdwt2_setup:DimensionError', ['Overlap larger than ' ...
           'the dimension of a facet along dimension(s): ', num2str(find(err_check))]);
