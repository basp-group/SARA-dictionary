...
% I : int[2]
%     Starting index of the tile (no overlap) [1,2].
% dims : int[2]
%     dimension of tbe underlying tile (no overlap) [1,2].
% Ncoefs : int[:, :]
%     Dimension of the wavelet facets for each level of the decomposition
%     level, for each dictionary [M(J+1),2] {from level 1 to J}.
% lo : array
%     Low-pass synthesis wavelet filter. [1,L]
% hi : array
%     High-pass synthesis wavelet filter. [1,L]
% J : int
%     Number of decomposition levels considered.
% pre_offset : int[2]
%     Number of coefficients to be cropped from the start of the
%     reconstructed facet [1,2].
% post_offset : int[2]
%     Number of coefficients to be cropped at the end of the
%     reconstructed facet [1,2].
%
% Returns
% -------
